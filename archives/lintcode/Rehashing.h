...
public:
    ...
    int hash(int v, int capacity) {
        return ((v % capacity) + capacity) % capacity;
    }

    vector<ListNode*> rehashing(vector<ListNode*> hashTable) {
        // write your code here
        int cap = 2 * (int)hashTable.size();
        vector<ListNode *> result(cap, NULL);
        for (int i = 0; i < hashTable.size(); i++) {
            ListNode *head = hashTable[i];
            while (head) {
                int pos = hash(head->val, cap);
                insertNode(result[ pos ], head->val);
                head = head->next;
            }
        }
        return result;
    ...
    void insertNode(ListNode *&node, int v) {
        ListNode *p = node;
        if (!node) {
            node = new ListNode(v);
            return;
